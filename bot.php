...
if (PHP_SAPI !== 'cli') {
    echo("请在命令行（CLI）环境中通过 screen 会话或 Docker 镜像运行此机器人：https://docs.madelineproto.xyz/docs/DOCKER.html！".PHP_EOL);
    die(1);
}
...
if ($process->join()) {
    echo("未安装 yt-dlp，请安装 yt-dlp！".PHP_EOL);
    die(1);
}
$process = Process::start([
    'ffmpeg',
...
$err = buffer($process->getStderr());
if ($process->join()) {
    echo("未安装 ffmpeg，请安装 ffmpeg！".PHP_EOL);
    die(1);
}
...
if ($sessionDir !== false) {
    $sessionFile = rtrim($sessionDir, '/\\') . '/bot.madeline';
} else {
    $sessionFile = 'bot.madeline';
}

MyEventHandler::startAndLoop($sessionFile, $settings);
